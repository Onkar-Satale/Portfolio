...
export default function ProjectDetailPage() {
  ...
  const project: Project | null = projectResponse as any || null;
  ...
  return (
    <div className="bg-white dark:bg-black transition-colors duration-300 min-h-screen">
        ...
        <div className="container mx-auto px-6">
            ...
            <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{duration: 0.5, delay: 0.4}} className="grid lg:grid-cols-3 gap-x-12 gap-y-8">
                ...
                <div className="space-y-8">
                    <div>
                        <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2"><FiUsers/> Team</h3>
                        <div className="mt-3 space-y-3">
                            {project.members && project.members.map(({member, role}) => {
                                if (!member) return null;
                                return (
                                <div key={member.id || Math.random()} className="flex items-center gap-3">
                                    {member.profileImage ? (
                            ...
                            {!project.members || project.members.length === 0 && (
                                <p className="text-slate-500 text-sm">Solo Developer / No Team Listed</p>
                            )}
                        </div>
                    </div>
                    <div>
      ...
      <style>{`
        ...
        .dark .project-description blockquote {
            border-left-color: #475569 !important;
            color: #94a3b8 !important;
        }
        .dark .project-description a {
          color: #60a5fa !important;
        }
        .dark .project-description a:hover {
